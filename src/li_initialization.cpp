...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  if (
    (lidar_type == VELO16 || lidar_type == OUST64 || lidar_type == HESA_IXT32) && cut_frame_init) {
    std::deque<PointCloudXYZI::Ptr> ptr;
    std::deque<double> timestamp_lidar;
    p_pre->processCutFramePcl2(msg, ptr, timestamp_lidar, cut_frame_num, scan_count);
...
void livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::SharedPtr & msg)
{
  ...
  if (cut_frame_init) {
    std::deque<PointCloudXYZI::Ptr> ptr;
    std::deque<double> timestamp_lidar;
    p_pre->processCutFrameLivox(msg, ptr, timestamp_lidar, cut_frame_num, scan_count);
